...
# );
#
# alter table public.knowledge_records add column if not exists uploader_user_id uuid;
# alter table public.knowledge_records add column if not exists reviewed_by uuid;
...
def get_supabase_auth_client():
    ...
    try:
        ...
        st.warning(f"Supabase Auth 客户端加载失败：{e}")
        return None
...
def _user_to_dict(user: Any) -> Dict[str, Any]:
    ...
    if isinstance(user, dict):
        return user
    out = {}
    for key in ["id", "email", "user_metadata"]:
        try:
...
def ensure_profile(user: Any) -> Dict[str, Any]:
    ...
    display_name = metadata.get("display_name") if isinstance(metadata, dict) else ""
    if not user_id:
        return {}
    client_sb = get_supabase_client()
    if client_sb is None:
        return {"id": user_id, "email": email, "display_name": display_name or email, "role": "user"}
    try:
        existing = client_sb.table("profiles").select("*").eq("id", user_id).limit(1).execute()
        if existing.data:
        ...
        client_sb.table("profiles").insert(payload).execute()
        return payload
    except Exception:
        return {"id": user_id, "email": email, "display_name": display_name or email, "role": "user"}


def _apply_auth_user(user: Any) -> bool:
    user_dict = _user_to_dict(user)
    user_id = user_dict.get("id", "")
    if not user_id:
        return False
    profile = ensure_profile(user_dict)
    st.session_state.auth_user = user_dict
    st.session_state.user_id = user_id
    st.session_state.user_email = user_dict.get("email", "")
    st.session_state.user_role = profile.get("role", "user") if isinstance(profile, dict) else "user"
...
def sign_up_user(email: str, password: str, display_name: str = "") -> Tuple[bool, str]:
    ...
    try:
        ...
        if user is not None:
            _apply_auth_user(user)
        return True, "注册请求已提交。如项目开启邮箱确认，请先检查邮箱。"
    except Exception as e:
...
def sign_in_user(email: str, password: str) -> Tuple[bool, str]:
    ...
    if client_auth is None:
        return False, "未配置 Supabase Auth。"
    try:
        res = client_auth.auth.sign_in_with_password({"email": email, "password": password})
        user = getattr(res, "user", None)
        if user is None:
            return False, "登录失败：未返回用户信息。"
        _apply_auth_user(user)
        return True, "登录成功。"
    except Exception as e:
...
def is_admin_user(user: Any = None) -> bool:
    if st.session_state.get("user_role") == "admin":
        return True
    user_dict = _user_to_dict(user) if user is not None else get_current_user()
    return bool(user_dict and st.session_state.get("user_role") == "admin")


def create_user_project(project_name: str, project_desc: str, project_type: str) -> bool:
    client_sb = get_supabase_client()
    user_id = st.session_state.get("user_id")
    if client_sb is None or not user_id:
        return False
    try:
        client_sb.table("user_projects").insert({
            "user_id": user_id,
        ...
        st.warning(f"项目保存失败：{e}")
        return False


def list_user_projects(all_users: bool = False) -> List[Dict[str, Any]]:
    client_sb = get_supabase_client()
    user_id = st.session_state.get("user_id")
    if client_sb is None or not user_id:
        return []
    try:
        query = client_sb.table("user_projects").select("*").order("created_at", desc=True).limit(200)
        if not all_users:
...
def save_recommendation_history(mode: str, targets: Dict[str, Any], result_df: pd.DataFrame, project_id: Optional[int] = None) -> bool:
    client_sb = get_supabase_client()
    user_id = st.session_state.get("user_id")
    if client_sb is None or not user_id:
        return False
    try:
        records = result_df.head(20).where(pd.notnull(result_df.head(20)), None).to_dict(orient="records")
        client_sb.table("recommendation_history").insert({
            "user_id": user_id,
            "project_id": project_id,
            "mode": mode,
            "target_json": targets or {},
            "result_json": {"rows": records},
        }).execute()
        return True
    except Exception as e:
        st.warning(f"推荐历史保存失败：{e}")
        return False


def list_recommendation_history(all_users: bool = False) -> List[Dict[str, Any]]:
    client_sb = get_supabase_client()
    user_id = st.session_state.get("user_id")
    if client_sb is None or not user_id:
        return []
    try:
        query = client_sb.table("recommendation_history").select("*").order("created_at", desc=True).limit(200)
        if not all_users:
            query = query.eq("user_id", user_id)
        return query.execute().data or []
    except Exception:
...
def save_user_report(report_title: str, report_type: str, report_content: str, project_id: Optional[int] = None) -> bool:
    client_sb = get_supabase_client()
    user_id = st.session_state.get("user_id")
    if client_sb is None or not user_id:
        return False
    try:
        client_sb.table("user_reports").insert({
            "user_id": user_id,
            "project_id": project_id,
...
def list_user_reports(all_users: bool = False) -> List[Dict[str, Any]]:
    client_sb = get_supabase_client()
    user_id = st.session_state.get("user_id")
    if client_sb is None or not user_id:
        return []
    try:
        query = client_sb.table("user_reports").select("*").order("created_at", desc=True).limit(200)
        if not all_users:
            query = query.eq("user_id", user_id)
        return query.execute().data or []
    except Exception:
...
def add_pending_knowledge(
    ...
    try:
        ...
        user_id = st.session_state.get("user_id")
        if user_id:
            payload["uploader_user_id"] = user_id
        try:
...
def init_usage_tracking() -> None:
    ...
    get_session_id()
    if not st.session_state.get("usage_session_recorded"):
        record_usage_event("session_start")
...
def get_supabase_status() -> Dict[str, Any]:
    ...
    client_ok = client_sb is not None
    table_ok = False
    if client_ok:
        try:
            client_sb.table("knowledge_records").select("id", count="exact").limit(1).execute()
            table_ok = True
        except Exception:
            table_ok = False
    status_text = "未配置"
    if configured and client_ok and table_ok:
        status_text = "已连接"
    elif configured:
        status_text = "异常"
    return {
...
def render_sidebar_status_panel() -> None:
    ...
    kstats = supa.get("stats", {})
    start_time = st.session_state.get("session_start_time")
    if isinstance(start_time, datetime):
        minutes = max(0, int((datetime.now() - start_time).total_seconds() // 60))
        session_duration = f"{minutes} min"
    else:
        session_duration = "--"
    model_loaded = "已加载" if isinstance(st.session_state.get("ml_bundle") or st.session_state.get("ml_training_bundle"), dict) else "未加载"

    st.markdown('<div class="sidebar-card">', unsafe_allow_html=True)
    st.markdown('<div class="sidebar-title">平台状态</div>', unsafe_allow_html=True)
    rows = [
        ("今日访问", usage.get("today_visits", "--")),
        ("累计访问", usage.get("total_visits", "--")),
        ("独立会话", usage.get("unique_sessions", "--")),
        ("当前会话", session_duration),
        ("数据库", supa.get("status_text", "未配置")),
        ("模型", model_loaded),
        ("公共知识", f"{kstats.get('approved', 0)} 条"),
        ("待审核", f"{kstats.get('pending', 0)} 条"),
    ]
    for label, value in rows:
        st.markdown(
            f'<div class="sidebar-status-row"><span>{html.escape(str(label))}</span><b>{html.escape(str(value))}</b></div>',
            unsafe_allow_html=True,
        )
...
def render_landing_page() -> None:
    st.markdown(
        ...
        @keyframes landingDrift {
            ...
            to { background-position: 30px 18px, -22px 28px, 0 0; }
        }
        .landing-nav {
            display: flex;
            align-items: center;
        ...
        .landing-cta-stack-wrap .stButton {
            width: auto;
        }
        .landing-cta-stack-wrap .stButton > button {
            min-width: 164px;
            height: 52px;
            padding: 0 24px;
            border-radius: 18px !important;
        ...
        .landing-cta-stack-wrap .stButton > button:hover {
        ...
        .landing-cta-stack-wrap .stButton:nth-of-type(2) > button {
            background: rgba(255,255,255,0.74) !important;
            color: #0f5572 !important;
            border: 1px solid rgba(14,165,233,0.35) !important;
        }
        .landing-cta-stack-wrap .stButton:nth-of-type(3) > button {
            background: rgba(255,255,255,0.42) !important;
            color: #25546d !important;
            border: 1px solid rgba(37,84,109,0.18) !important;
            box-shadow: none;
        }
        .hero-visual {
            position: relative;
        ...
        .nano-dot.a { top: 88px; left: 88px; }
        .nano-dot.b { top: 138px; right: 84px; background: #a78bfa; }
        .nano-dot.c { bottom: 104px; left: 132px; background: #34d399; }
        .nano-dot.d { bottom: 76px; right: 116px; background: #fbbf24; }
        .hero-panel {
            position: absolute;
            left: 44px;
            right: 44px;
        ...
        .hero-bar:nth-child(2) { width: 76%; opacity: 0.74; }
        .hero-bar:nth-child(3) { width: 54%; opacity: 0.58; }
        ...
        .value-icon, .feature-icon-lg {
            ...
            place-items: center;
            border-radius: 18px;
            background: linear-gradient(135deg, rgba(14,165,233,0.16), rgba(124,58,237,0.13));
            font-size: 25px;
            margin-bottom: 18px;
        }
        .value-card h3 {
        ...
        .landing-card {
            ...
            padding: 28px;
            min-height: 245px;
            box-shadow: 0 18px 50px rgba(15,55,87,0.08);
        ...
        .landing-feature-actions .stButton > button {
            min-height: 48px;
            border-radius: 16px !important;
        ...
        <div class="landing-shell">
            <div class="landing-nav">
                <div class="landing-brand">仿生 rHDL 纳米制剂开发助手</div>
                <div class="landing-links">
                    <span>平台能力</span><span>技术架构</span><span>登录</span><span>注册</span><span>进入工作台</span>
                </div>
            </div>
            <div class="landing-hero">
                <div>
                    <div class="landing-eyebrow">AI + BioPharma Formulation Intelligence</div>
                    <h1>仿生 rHDL 纳米制剂开发助手</h1>
                    <p>AI 驱动的文献解析、处方预测与实验辅助平台。</p>
                    <div class="landing-hero-note">将文献数据库、CatBoost 正向模型、Optuna 反向搜索与实验诊断串联成一条可复用的纳米制剂研发流程。</div>
        """,
        unsafe_allow_html=True,
    )
    st.markdown('<div class="landing-cta-stack-wrap">', unsafe_allow_html=True)
    if st.button("进入工作台", use_container_width=True, type="primary", key="landing_enter_workspace_btn"):
        set_route("workspace")
        st.rerun()
    if st.button("注册账号", use_container_width=True, key="landing_register_btn"):
        set_route("register")
        st.rerun()
    if st.button("登录", use_container_width=True, key="landing_login_btn"):
        set_route("login")
        st.rerun()
    st.markdown(
        """
                ...
                <div class="hero-visual">
                    <div class="nano-orbit"></div>
                    <div class="nano-orbit two"></div>
                    <div class="nano-dot a"></div>
                    <div class="nano-dot b"></div>
                    <div class="nano-dot c"></div>
                    <div class="nano-dot d"></div>
                    <div class="hero-panel">
                        <div class="hero-panel-title">rHDL formulation intelligence</div>
                        <div class="hero-bars"><div class="hero-bar"></div><div class="hero-bar"></div><div class="hero-bar"></div></div>
                    </div>
                </div>
            </div>
        """,
    ...
    st.markdown(
        """
        <div class="landing-section-kicker">Product Value</div>
        <div class="value-grid">
            <div class="value-card"><div class="value-icon">🧬</div><h3>把文献变成可计算数据</h3><p>围绕 rHDL 处方组成、工艺条件和理化指标建立结构化研发知识底座。</p></div>
            <div class="value-card"><div class="value-icon">⚗️</div><h3>从目标指标反推候选处方</h3><p>通过 CatBoost 正向预测与 Optuna / Pareto 搜索，输出可排序、可解释的推荐方案。</p></div>
            <div class="value-card"><div class="value-icon">📊</div><h3>让实验决策有依据</h3><p>结合适用域距离、相似文献样本和模型报告，辅助判断外推风险与验证优先级。</p></div>
        </div>
        <div class="landing-section-kicker">Product Modules</div>
        """,
        unsafe_allow_html=True,
    )

    cards = [
        ("📖", "文献解析中心", "提炼摘要、解析 Methods、抽取处方和评价字段。", "文献解析中心"),
    ...
    for icon, title, desc, _page in cards:
        st.markdown(
            f'<div class="landing-card"><div class="feature-icon-lg">{icon}</div><h3>{html.escape(title)}</h3><p>{html.escape(desc)}</p><div class="feature-btn">进入模块 →</div></div>',
            unsafe_allow_html=True,
        )
    st.markdown("</div>", unsafe_allow_html=True)
    st.markdown('<div class="landing-feature-actions">', unsafe_allow_html=True)
    action_rows = [cards[:3], cards[3:]]
    for row_idx, row in enumerate(action_rows):
        cols = st.columns(3)
        for col_idx, (_icon, title, _desc, page) in enumerate(row):
            with cols[col_idx]:
                if st.button(f"进入{title}", use_container_width=True, key=f"landing_feature_action_{row_idx}_{col_idx}"):
    ...
    st.markdown(
        """
            <div class="landing-section">
                <div class="landing-section-kicker">Workflow</div>
                <h2>从文献证据到实验验证</h2>
                <div class="workflow">
                    <span>文献数据</span><span>字段提取</span><span>模型训练</span><span>目标输入</span><span>反向推荐</span><span>实验验证</span>
                </div>
            </div>
            <div class="landing-section">
                <div class="landing-section-kicker">Architecture</div>
                <h2>技术架构</h2>
                <div class="workflow">
                    <span>Streamlit</span><span>Supabase Auth / PostgreSQL</span><span>CatBoost</span><span>Optuna / Pareto</span><span>OpenAI-compatible LLM Agent</span><span>Plotly</span>
                </div>
            </div>
            <div class="landing-cta">
                <h2>开始构建你的 rHDL 处方推荐流程</h2>
                <p>立即体验文献解析、处方预测与实验辅助能力，让每一次实验迭代更有方向。</p>
            </div>
        </div>
        """,
        unsafe_allow_html=True,
    ...
    with cta_mid:
        if st.button("注册并进入工作台", use_container_width=True, key="landing_bottom_register_btn"):
            set_route("register")
            st.rerun()
...
def render_login_page() -> None:
    ...
    with c1:
        if st.button("登录", use_container_width=True, type="primary", key="login_submit_btn"):
            ok, msg = sign_in_user(email, password)
            if ok:
                st.success(msg)
                set_route("workspace")
                st.rerun()
                ...
                st.error(msg)
    with c2:
        if st.button("去注册", use_container_width=True, key="login_to_register_btn"):
            set_route("register")
            st.rerun()
    with c3:
        if st.button("返回首页", use_container_width=True, key="login_back_landing_btn"):
            set_route("landing")
            st.rerun()
    ...
    with c1:
        if st.button("注册", use_container_width=True, type="primary", key="register_submit_btn"):
            if password != password2:
                ...
                ok, msg = sign_up_user(email, password, display_name)
                if ok:
                    st.success(msg)
                    ...
                    st.error(msg)
    with c2:
        if st.button("去登录", use_container_width=True, key="register_to_login_btn"):
            set_route("login")
            st.rerun()
    with c3:
        if st.button("返回首页", use_container_width=True, key="register_back_landing_btn"):
            set_route("landing")
            st.rerun()


def render_workspace_topbar() -> None:
...
def render_profile_page() -> None:
    render_module_header("我的空间", "管理长期项目、推荐历史和报告记录。")
    ...
    with st.container(border=True):
        ...
        p1, p2 = st.columns(2)
        with p1:
            project_name = st.text_input("项目名称", key="profile_project_name_input")
            project_type = st.selectbox("项目类型", ["处方开发", "数据库分析", "实验设计", "综合智能体"], key="profile_project_type_select")
        with p2:
            project_desc = st.text_area("项目描述", height=110, key="profile_project_desc_textarea")
        if st.button("保存项目", use_container_width=True, key="profile_save_project_btn"):
            if project_name.strip() and create_user_project(project_name.strip(), project_desc, project_type):
                st.success("项目已保存。")
                st.rerun()
            else:
                st.warning("请填写项目名称，或检查 Supabase 配置。")

    tabs = st.tabs(["我的项目", "推荐历史", "报告"])
    with tabs[0]:
        projects = list_user_projects(all_users=is_admin_user())
        if projects:
            safe_dataframe(pd.DataFrame(projects), use_container_width=True, height=320)
        else:
            st.info("暂无项目。")
    with tabs[1]:
        history = list_recommendation_history(all_users=is_admin_user())
        if history:
            safe_dataframe(pd.DataFrame(history), use_container_width=True, height=360)
        else:
            st.info("暂无推荐历史。")
    with tabs[2]:
        reports = list_user_reports(all_users=is_admin_user())
        if reports:
            st.markdown("#### 已保存报告")
            safe_dataframe(pd.DataFrame(reports), use_container_width=True, height=260)
        report_title = st.text_input("报告标题", key="profile_report_title_input")
        report_content = st.text_area("报告内容", height=160, key="profile_report_content_textarea")
        if st.button("保存报告", use_container_width=True, key="profile_save_report_btn"):
            if report_title.strip() and save_user_report(report_title.strip(), "manual", report_content):
                st.success("报告已保存。")
                st.rerun()
            else:
                st.warning("请填写报告标题，或检查 Supabase 配置。")


def render_admin_page() -> None:
    render_module_header("管理员后台", "知识库审核、访问统计和用户项目只读概览。")
    if not is_admin_user():
        st.warning("当前用户没有管理员权限。")
        return
    stats = get_usage_stats()
    s1, s2, s3 = st.columns(3)
    s1.metric("今日访问", stats.get("today_visits", "--"))
    s2.metric("累计访问", stats.get("total_visits", "--"))
    s3.metric("独立会话", stats.get("unique_sessions", "--"))
    st.markdown("#### 公共知识库审核")
    render_knowledge_admin_panel(location_key="admin_page")
    st.markdown("#### 所有用户项目")
    projects = list_user_projects(all_users=True)
    if projects:
        safe_dataframe(pd.DataFrame(projects), use_container_width=True, height=360)
    else:
        st.info("暂无项目记录。")
...
def render_sidebar() -> None:
    with st.sidebar:
        ...
        if st.session_state.get("is_logged_in"):
            st.caption(f"用户：{st.session_state.get('user_email', '')}")
            st.caption(f"角色：{st.session_state.get('user_role', 'user')}")
        ...
        if st.button("提交到公共知识库待审核", use_container_width=True, key="sidebar_submit_pending_kb_btn"):
            ...
            if submitted:
                ...
                record_usage_event("knowledge_upload", detail={"count": submitted})
            else:
...
def render_real_ml_recommender_panel(df_main: Optional[pd.DataFrame]) -> None:
    ...
    status_cards = [
        ("当前模型状态", status_label),
        ("已训练目标", str(model_count)),
        ("模型版本", model_version),
        ("模型文件", "已存在" if saved_exists else "不存在"),
    ]
    card_html = ['<div class="status-grid">']
    for label, value in status_cards:
        card_html.append(
            f'<div class="status-card"><div class="label">{html.escape(str(label))}</div><div class="value">{html.escape(str(value))}</div></div>'
        )
    ...
    st.markdown("#### 模型管理")
    c_train, c_reload, c_delete = st.columns([1, 1, 1], gap="medium")
    with c_train:
        if st.button("训练并保存", use_container_width=True, key="real_ml_train_save_btn", disabled=not has_data):
            try:
    ...
    with c_reload:
        if st.button("重新加载", use_container_width=True, key="real_ml_reload_bundle_btn"):
            bundle = load_model_bundle()
            if isinstance(bundle, dict):
                _set_active_ml_bundle(bundle, "local_file")
                st.session_state.ml_recommendation_df = None
    ...
    with c_delete:
        if st.button("删除模型", use_container_width=True, key="real_ml_delete_bundle_btn"):
            try:
    ...
    st.markdown(
        """
        <div class="info-card">
            <div class="info-card-title">模型说明</div>
            <div class="info-card-body">
                训练器：CatBoostRegressor<br>
                优化器：Optuna TPE / Pareto 多目标<br>
                评价方式：Holdout MAE/R² + K 折交叉验证<br>
                推荐可信度：适用域距离 + 相似文献样本
            </div>
        </div>
        """,
    ...
    with opt_tab:
        ...
        if st.button("运行贝叶斯优化推荐", use_container_width=True, key="real_ml_bayesian_recommend_btn", disabled=not has_bundle):
            try:
                ...
                record_usage_event("ml_recommendation", page_name=st.session_state.get("active_page", ""), detail={"mode": "bayesian"})
            except Exception as e:
        ...
        if isinstance(rec_df, pd.DataFrame) and not rec_df.empty:
            ...
            if st.session_state.get("is_logged_in"):
                ...
                if st.button("保存到我的推荐历史", use_container_width=True, key="save_bayesian_recommendation_history_btn"):
                    if save_recommendation_history("bayesian", save_targets, rec_df):
                        st.success("已保存到我的推荐历史。")
                    else:
                        st.warning("保存失败，请检查登录状态或 Supabase 配置。")
    ...
    with pareto_tab:
        ...
        if st.button("运行 Pareto 多目标优化", use_container_width=True, key="real_ml_pareto_recommend_btn", disabled=not has_bundle):
            try:
                ...
                record_usage_event("ml_recommendation", page_name=st.session_state.get("active_page", ""), detail={"mode": "pareto"})
            except Exception as e:
        ...
        if isinstance(pareto_df, pd.DataFrame) and not pareto_df.empty:
            ...
            if st.session_state.get("is_logged_in"):
                ...
                if st.button("保存到我的 Pareto 推荐历史", use_container_width=True, key="save_pareto_recommendation_history_btn"):
                    if save_recommendation_history("pareto", save_targets, pareto_df):
                        st.success("已保存到我的推荐历史。")
                    else:
                        st.warning("保存失败，请检查登录状态或 Supabase 配置。")
...
def render_formulation_design_page() -> None:
    ...
    render_module_header(
        "rHDL纳米制剂处方设计",
        "从目标指标出发，使用真实Excel数据训练或加载模型，并进行处方/工艺参数反向推荐。",
    )
...
def render_real_catboost_prediction_panel(df_main: Optional[pd.DataFrame]) -> None:
    ...
    if st.button("运行真实 CatBoost 正向预测", use_container_width=True, key="predict_real_run_btn"):
        ...
        try:
            ...
            if st.session_state.get("is_logged_in"):
                if st.button("保存本次预测记录", use_container_width=True, key="predict_real_save_report_btn"):
                    content = json.dumps({"candidate": candidate, "predictions": preds, "applicability": ad}, ensure_ascii=False, default=str)
                    if save_user_report("真实 CatBoost 正向预测", "prediction", content):
                        st.success("预测记录已保存到我的报告。")
                    else:
                        st.warning("保存失败，请检查 Supabase 配置。")
        except Exception as e:
...
def render_page_navigation() -> str:
    ...
    if st.session_state.get("last_recorded_page") != current_page:
        record_usage_event("page_view", page_name=current_page)
...
def render_standalone_admin_route() -> None:
    if not st.session_state.get("is_logged_in"):
        set_route("login")
        st.rerun()
    render_workspace_topbar()
...
def main() -> None:
    ...
    if route == "landing":
        ...
        render_standalone_profile_route()
    elif route == "admin":
        render_standalone_admin_route()
